<template>
  <el-card class="box-card">
    ...
    <el-table strip border :data="data" style="width: 100%">
    ...
    >
      <el-form :model="form" label-width="80px" ref="editValidate">
        <el-form-item label="用户名" prop="username">
          <!-- readonly -->
          <el-input v-model="form.username" autocomplete="off"></el-input>
        </el-form-item>
  ...
  methods: {
    async loadData() {
      ...
      var response = await this.$http.get(
        `users?pagenum=${this.pagenum}&pagesize=${this.pagesize}&query=${this.inputVal}`
      );
    ...
    async addUser() {
      this.$refs.addValidate.validate(async valid => {
        if (valid) {
          ...
          if (status === 201) {
            this.$message.success(msg);
            this.loadData(), (this.adddialogFormVisible = false);
          } else {
            this.$message.error(msg);
          }
